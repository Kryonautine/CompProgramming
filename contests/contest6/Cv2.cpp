...
int main() {
    ...
    for (int i = 0 ; i < t ; i++) {
        ...
        for (long long k = 0; k < n + m + 1; k++) {
            if (prog_skill[k] > test_skill[k]) {
                if (n_i > 0) {team_skill += prog_skill[k]; n_i--;}
                else {team_skill += test_skill[k];}
            }
            else {
                if (m_i > 0) {team_skill += test_skill[k]; m_i--;}
                else {team_skill += prog_skill[k];}
            }
        }
